...
bool NetworkService::connectSta(const String& ssid, const String& pass) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // background). configTzTime() sets the TZ env var from the POSIX
    // string and applies its DST rule automatically, so localtime_r()
    // elsewhere reflects correct local time year-round.
    configTzTime(settings.tz_string.c_str(), "pool.ntp.org", "time.nist.gov");
...
void NetworkService::applySettings(const NetworkSettings& new_settings) {
  ...
  if (!wifi_changed) {
    // A TZ-only (or MQTT-only) change still needs configTzTime() re-armed
    // so a timezone edit takes effect immediately, without disturbing the
    // WiFi link. Cheap and idempotent to call again even if STA isn't
    // connected yet - it'll just apply once it is.
    configTzTime(settings.tz_string.c_str(), "pool.ntp.org", "time.nist.gov");
...
void NetworkService::loadSettings() {
  ...
  settings.tz_string = prefs.getString("tz_string", TZ_STRING_DEFAULT);
  prefs.end();
...
void NetworkService::saveSettings() {
  ...
  prefs.putString("tz_string", settings.tz_string);
  prefs.end();
